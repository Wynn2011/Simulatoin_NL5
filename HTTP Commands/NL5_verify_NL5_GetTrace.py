"""Step verification: NL5_GetTrace

Objective:
  Get the index and handle of the trace named "V(out)".

Command template from doc:
  http://localhost/?NL5_GetTrace,V(out)
...
def main() -> None:
    ...
    sc, body, url = http_get('?'+"NL5_GetTrace,V(out)".lstrip('?'))
    print("NL5_GetTrace:")
    print("  url:", url)
    print("  status:", sc)
    print("  body:", body)
