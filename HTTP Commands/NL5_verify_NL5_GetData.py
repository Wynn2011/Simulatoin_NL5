"""Step 31 verification: NL5_GetData

Objective:
  Get interpolated data for trace "V(out)" between 0s and 0.01s with a step of 10u.

Command template from doc:
  http://localhost/?NL5_GetData,V(out),from=0,to=0.01,step=10u
...
def main() -> None:
    ...
    # Send command
    cmd = "http://localhost/?NL5_GetData,V(out),from=0,to=0.01,step=10u"
